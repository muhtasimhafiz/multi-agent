"""Dense joint training with NO alpha decay.

target_active=4 of each type (collision-rich), but alpha is held constant
at 0.1 throughout training. The hypothesis: alpha decay was prematurely
locking B's Q-table into the "always cross" basin. Without it, Q-values
stay reactive to ongoing collision experience and B can keep adapting.
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "outputs_dense_noalpha")
...
def main():
    ...
    # No alpha decay: alpha_min defaults to alpha so it stays fixed at 0.1.
    q_kwargs = dict(
        q_init=50.0,
        alpha=0.1,                 # FIXED, no decay
        gamma=0.99,
        eps=0.2, eps_min=0.1, eps_decay_steps=600_000,
    )

    print("=== Dense joint training, NO alpha decay ===")
