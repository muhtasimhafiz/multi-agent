...
def main():
    ...
    learners = {
        TYPE_A: QLearner(q_init=0.0, alpha=0.15, alpha_min=0.15, alpha_decay_steps=1,
                         gamma=0.95, eps=1.0, eps_min=0.01, eps_decay_steps=5_000),
        TYPE_B: QLearner(q_init=0.0, alpha=0.15, alpha_min=0.15, alpha_decay_steps=1,
                         gamma=0.95, eps=1.0, eps_min=0.01, eps_decay_steps=5_000),
    }

    print('STAGE 1: A solo, 5k steps, alpha=0.15')
    _ = train(num_steps=5_000, log_window=1_000,
          env_kwargs={**base, 'enabled_types': (TYPE_A,), 'target_active': 1},
          learners=learners, verbose=True)

    print('\nSTAGE 2: B solo, 5k steps, alpha=0.15')
    reset(learners[TYPE_B], 0.15, 1.0, 0.01, 5_000)
    _ = train(num_steps=5_000, log_window=1_000,
